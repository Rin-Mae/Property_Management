    ...
    <script type="module">
        ...
        async function loadProcessingRequests() {
            try {
                ...
                allProcessingRequests = allRequests.filter(r => r.status === 'approved');

                document.getElementById('loading').style.display = 'none';

                if (allProcessingRequests.length === 0) {
                    document.getElementById('emptyState').style.display = 'block';
                    return;
                }

                renderProcessingRequests(allProcessingRequests);
                document.getElementById('requestsTable').style.display = 'table';
            } catch (error) {
                console.error('Failed to load processing requests:', error);
                document.getElementById('loading').textContent = 'Failed to load requests. Please refresh the page.';
            }
        }

        function renderProcessingRequests(requests) {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-release" onclick="readyForRelease(${request.id})">Ready for Release</button>
                        </div>
                    </td>
                `;
        ...
        window.applySearch = function () {
            ...
            const filtered = allProcessingRequests.filter(request => {
                const matchesSearch =
                    !searchTerm ||
            ...
            renderProcessingRequests(filtered);
        };
        ...
        window.readyForRelease = async function (requestId) {
            if (!confirm('Mark this request as ready for release?')) return;
            ...
            try {
                ...
                console.error('Failed to mark request as ready for release:', error);
